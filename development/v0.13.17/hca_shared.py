...
def _hca137_product_details(product: dict[str, Any]) -> dict[str, Any]:
...
def _hca138_product_visuals(product: dict[str, Any]) -> dict[str, Any]:
...
def _wsk_product_configs(product: dict[str, Any]) -> list[dict[str, Any]]:
    ...
    common = {**_hca137_product_details(product), "product_id": product_id, "name": name, "sku": sku, "image": image, "weight": str(product.get("weight") or ""), "dimensions": product.get("dimensions") if isinstance(product.get("dimensions"), dict) else {}, "stock_status": str(product.get("stock_status") or ""), "manufacturer": manufacturer, "supplier": supplier, "wsk": True, "price": product.get("price"), "regular_price": product.get("regular_price"), "areas": _as_data(meta.get("werbeartikel_areas"), []) if isinstance(_as_data(meta.get("werbeartikel_areas"), []), list) else []}
    result: list[dict[str, Any]] = []
    if colors:
    ...
    with _db() as conn:
        ...
        for r in items:
            ...
            conn.execute("INSERT INTO sales_order_items(id,sales_order_id,position_no,item_type,product_id,sku,description,quantity,unit,unit_price,discount_pct,tax_rate,config_json,created_at,updated_at) VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)",(
                str(uuid.uuid4()),oid,r["position_no"],r["item_type"],r["product_id"],r["sku"],r["description"],*_hca136_quote_tier_values(r,payload.get("selected_quantity_tier")),r["discount_pct"],r["tax_rate"],_json(_hca136_quote_tier_config(r,payload.get("selected_quantity_tier"))),now,now))
    ...
    for item in items:
        ...
        for step in steps:
            ...
            for size,raw_qty in sizes.items():
                ...
                if not qty: continue
                target.append({"article":item["description"],"sku":str(size_skus.get(size) or item["sku"] or ""),"color":color,"size":str(size),"quantity":qty,"team_name":"","personalization_name":"","number_text":"","techniques":technique,"position":str((step or {}).get("position") or ""),"motif":str((step or {}).get("motif") or ""),"shipping_group":"","product_id":str(item["product_id"] or ""),"packaging_unit":max(1,int(float(config.get("packaging_unit") or 1))),"production_mode":str(config.get("production_mode") or "")})
...
def _shop_int(value: Any, default: int = 0) -> int:
    try:
        return int(value)
    except (TypeError, ValueError):
        return default
...
def _hca137_datasheet_specs(document: dict[str, Any]) -> list[dict[str, Any]]:
    ...
    for item in document.get("items") or []:
        ...
        lines=[]
        for label,value in (
            ("Artikelnummer",sku),("Hersteller / Marke",details.get("manufacturer")),
        ...
        if loaded:
            commands=["0 0 0 rg","BT /F2 15 Tf 1 0 0 1 46 672 Tm (HAUPTBILD UND PRODUKTBILDER) Tj ET",
...
def _commercial_pdf_bytes(kind: str, document: dict[str, Any], address: dict[str, Any]) -> bytes:
    ...
    for page_index, page_rows in enumerate(pages, 1):
        ...
        page_contents.append("\n".join(commands).encode("cp1252", "replace"))
    return _hca137_quote_pdf(page_contents, document) if is_quote else _letterhead_pdf(page_contents)
